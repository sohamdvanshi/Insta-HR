...
export default function AdminPage() {
  ...
  const fetchAll = async (token: string) => {
    try {
      ...
      const [statsData, jobsData, usersData] = await Promise.all([statsRes.json(), jobsRes.json(), usersRes.json()]);
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
            {[
              { label: 'Total Jobs', value: stats.totalJobs, emoji: 'briefcase' },
              { label: 'Total Users', value: stats.totalUsers, emoji: 'users' },
              { label: 'Applications', value: stats.totalApplications, emoji: 'clipboard' },
              { label: 'Revenue', value: "Rs." + stats.revenue, emoji: 'money' }
            ].map(stat => (
        ...
        <div className="flex gap-2 mb-6">
          {['dashboard', 'jobs', 'users'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={"px-4 py-2 rounded-xl font-medium text-sm capitalize transition-colors " + (activeTab === tab ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50')}>
              {tab}
            </button>
          ))}
        </div>
                ...
                <div key={user.id} className="p-6 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      ...
                      className="text-xs border border-gray-200 rounded-lg px-2 py-1 outline-none focus:border-blue-500"
                    >
                    ...
                    <button onClick={() => toggleUserActive(user.id)}
                      className={"px-3 py-1 text-xs rounded-lg " + (user.isActive ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-green-50 text-green-600 hover:bg-green-100')}>
