...
fs.mkdirSync('src/app/saved-jobs', { recursive: true });
fs.writeFileSync('src/app/saved-jobs/page.tsx', savedPage);
console.log('✅ Saved jobs page created at src/app/saved-jobs/page.tsx');

// ─── UPDATE JOBS LISTING PAGE to add bookmark button ───
console.log('\n📝 Now updating jobs listing to add bookmark button...');

let jobsPage = fs.readFileSync('src/app/jobs/page.tsx', 'utf8');

if (!jobsPage.includes('savedJobs') && !jobsPage.includes('bookmark')) {
  // Add savedJobs state after existing useState declarations
  jobsPage = jobsPage.replace(
    `const [loading, setLoading] = useState(true)`,
    `const [loading, setLoading] = useState(true)
  ...
  // Add fetchSavedJobIds function before return
  jobsPage = jobsPage.replace(
    `const fetchJobs`,
  ...
  // Add bookmark button to each job card - find the Apply button and add bookmark next to it
  jobsPage = jobsPage.replace(
    `href={'/jobs/' + job.id}`,
    `href={'/jobs/' + job.id}`
  );

  // Find where the job card apply button is and inject bookmark
  if (jobsPage.includes("View Job") || jobsPage.includes("Apply Now") || jobsPage.includes("View Details")) {
    jobsPage = jobsPage.replace(
      /(<(?:Link|a)[^>]*>(?:View Job|Apply Now|View Details)<\/(?:Link|a)>)/,
      `$1
                  <button onClick={(e) => toggleSave(e, job.id)}
                    className={"ml-2 p-2 rounded-xl border transition-colors " + (savedJobIds.has(job.id) ? 'bg-blue-50 border-blue-300 text-blue-600' : 'border-gray-200 text-gray-400 hover:border-blue-300 hover:text-blue-500')}
  ...
  fs.writeFileSync('src/app/jobs/page.tsx', jobsPage);
  console.log('✅ Jobs page updated with bookmark button');
} else {
  console.log('ℹ️  Jobs page already has bookmark functionality');
}

// ─── UPDATE NAVBAR to add Saved Jobs link for candidates ───
let navbar = fs.readFileSync('src/components/Navbar.tsx', 'utf8');
if (!navbar.includes('saved-jobs')) {
  ...
  console.log('✅ Navbar updated with Saved Jobs link');
} else {
  console.log('ℹ️  Navbar already has Saved Jobs link');
}

console.log('\n🎉 All frontend done!');
console.log('Pages created:');
console.log('  → http://localhost:3000/saved-jobs');