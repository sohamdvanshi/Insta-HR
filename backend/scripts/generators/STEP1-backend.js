// Generator: scaffolds AI screening controller+routes (original step 1)
// Run from backend/: node scripts/generators/STEP1-backend.js
const fs = require('fs');
...
exports.screenCandidates = async (req, res) => {
  try {
    ...
    const applications = await Application.findAll({ where: { jobId }, include: [{ model: User, as: 'candidate', attributes: ['id','name','email'] }] });
    if (!applications.length) return res.json({ success: true, data: [], message: 'No applications yet' });
    ...
    const results = applications.map(app => {
      ...
      return { applicationId: app.id, applicationStatus: app.status, userId: app.userId, name: user.name||'Unknown', email: user.email||'', photoUrl: profile.photoUrl||null, headline: profile.headline||'', currentLocation: profile.currentLocation||'', yearsOfExperience: candidateExp, resumeUrl: profile.resumeUrl||null, scores: { skillMatch: skillPct, experienceMatch: expPct, keywordRelevance: kwPct, overall }, matchedSkills: getMatchedSkills(jobSkills, candidateSkills), missingSkills: getMissingSkills(jobSkills, candidateSkills), label: getLabel(overall), labelColor: getLabelColor(overall), appliedAt: app.createdAt };
    });
    results.sort((a, b) => b.scores.overall - a.scores.overall);
    const summary = { total: results.length, excellent: results.filter(r=>r.scores.overall>=80).length, good: results.filter(r=>r.scores.overall>=60&&r.scores.overall<80).length, partial: results.filter(r=>r.scores.overall>=40&&r.scores.overall<60).length, low: results.filter(r=>r.scores.overall<40).length, avgScore: results.length?Math.round(results.reduce((s,r)=>s+r.scores.overall,0)/results.length):0, jobTitle: job.title, jobSkills: job.skills||[], jobExpMin: job.experienceMin||0 };
    res.json({ success: true, data: results, summary });
  } catch (error) { console.error('AI Screening error:', error); res.status(500).json({ success: false, message: error.message }); }
};
...
fs.writeFileSync(path.join(srcBase, 'controllers/aiScreening.controller.js'), controller);
...
`;
fs.writeFileSync(path.join(srcBase, 'routes/aiScreening.routes.js'), routes);
console.log('✅ Routes created');

let server = fs.readFileSync(path.join(srcBase, 'server.js'), 'utf8');
if (!server.includes('aiScreening')) {
  server = server.replace("app.use('/api/v1/jobs-actions'", "app.use('/api/v1/ai', require('./routes/aiScreening.routes'));\napp.use('/api/v1/jobs-actions'");
  fs.writeFileSync(path.join(srcBase, 'server.js'), server);
  console.log('✅ Route registered in server.js');
} else { console.log('ℹ️  Route already registered'); }
